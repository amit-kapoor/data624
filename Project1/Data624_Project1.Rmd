...
The dataset contains residential power usage for January 1998 until December 2013. Its monthly data from 1998 and power consumed is in KWH column.
...
Seeing the plot closely, it is apparent that there is an outlier and a missing entry too.
...
It is apparent here that the data is recorded on different time intervals for pipe1 and pipe2. For pipe1 it shows records for multiple time intervals within an hour i.e. not evenly distributed while for pipe2 it seems hourly recorded. We can check the daily frequency count below for both datasets to confirm this finding.
...
In this step we will make pipe1 data evenly distributed (per hour) for every day. We will take mean of records per hour and then recreate the `Date Time` column. Nest we will do the same count comparison that was done earlier and check number of records per day.
...
pipe.forecast <- function(timeseries, pipe_num) {
  ...
  # forecast h=24*7=168
  atm.ets.fcst <- forecast(ets.model, h=168)
  atm.arima.fcst <- forecast(arima.model, h=168)
...
```{r}
pipe1.ts %>% ets(lambda = pipe1.lambda)
```



```{r}
pipe1.ts %>% auto.arima(lambda = pipe1.lambda)
```
...
```{r}
pipe2.ts %>% ets(lambda = pipe2.lambda)
```



```{r}
pipe2.ts %>% auto.arima(lambda = pipe2.lambda)
```
